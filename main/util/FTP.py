...
class AdamWP(Optimizer):
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        weight_decay=weight_decay, amsgrad=amsgrad)
        super(AdamWP, self).__init__(params, defaults)

        # initialize FTP
        self.ftp = FTP(k, exclude_set=exclude_set)

    def __setstate__(self, state):
        super(AdamWP, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', False)
    ...
    def adamw(self, group,
        ...
        for param, name, pre in zip(group['params'], group['name'], group['pre']):
            ...
            grad = param.grad
            if weight_decay != 0:
                grad = grad.add(param, alpha=weight_decay)  # This line applies weight decay to the gradient
            exp_avg = exp_avgs[i]
            exp_avg_sq = exp_avg_sqs[i]
            step = state_steps[i]
            if amsgrad:
            ...
            new_p = self.ftp.step(name, param, pre, d_p)  # Note: Weight decay is not added here anymore
            if new_p is None:
                new_p = param - d_p
            param.copy_(new_p)
            i += 1
